...
class VoiceAssistant(utils.EventEmitter[EventTypes]):
    ...
    def _synthesize_agent_reply(self) -> None:
        ...
        # Instead of creating a new task here, we'll use a coroutine
        self._agent_reply_coro = self._synthesize_answer_coro(
            self._agent_reply_task, new_handle
        )

        # Schedule the coroutine to run in the next iteration of the event loop
        self._loop.call_soon(asyncio.create_task, self._agent_reply_coro)

    async def _synthesize_answer_coro(
        self, old_task: asyncio.Task[None], handle: SpeechHandle
    ) -> None:
        # This method remains largely the same as _synthesize_answer_task,
        # but is now a coroutine instead of being wrapped in a task
...
async def _llm_stream_to_str_iterable(
    ...
    try:
        async for chunk in stream:
            ...
            # Envie o texto em chunks menores (por exemplo, a cada 50 caracteres)
            while len(buffer) >= 50:
                yield buffer[:50]
                buffer = buffer[50:]

        # Envie o restante do buffer
        if buffer:
            yield buffer
    except Exception as e:
        logger.error(f"Error in _llm_stream_to_str_iterable: {e}")
